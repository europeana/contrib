...
public abstract class Task {
	...
	public enum Status { 
		OK, WARNINGS, ERRORS, CANCELED, FAILED; 
	...
	protected long retryDelay = 300; // Delay sleep time in seconds
	...
	protected boolean isTimeToRetry(Calendar calendar) {
		...
		if(retries < maxRetries && failTimeDiffInMillis > minRetryDelayInMillis) {
			return true;
		}
		else {
			return false;
	...
	public boolean equalsAction(Task otherTask) {
		if(CompareUtil.compareObjectsAndNull(taskClass, otherTask.getTaskClass())
