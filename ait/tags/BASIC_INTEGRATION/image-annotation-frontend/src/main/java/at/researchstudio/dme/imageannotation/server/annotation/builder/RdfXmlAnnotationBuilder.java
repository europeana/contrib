...
import at.researchstudio.dme.imageannotation.client.image.shape.Line;
...
public class RdfXmlAnnotationBuilder {
	...
	private static void toRdf(RepositoryConnection con, ValueFactory vFactory, ImageAnnotation 
	    ...
	    // replies
	    for(Object reply : annotation.getReplies()) toRdf(con,vFactory,(ImageAnnotation)reply); 
	}
	...
	private static void createSvgSymbol(Element symbol, Element use, Shape shape) {	
		...
		// implement shape.getSvg() and override the method for each shape. The problem
		// is the shapes are also used on the client and therefore compiled to javascript
		// which is not possible when using jdom.
		...
		// a cross is presented using 4 lines 
			...
			for(Line line : cross.getLines()) {
				...
				lineEl.setAttribute(new Attribute(SVG_STROKE_WIDTH, String.valueOf(shape.getStrokeWidth())));		
