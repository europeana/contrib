 ...
 */
	...
	public List<DataProvider> loadDataProviders(File dataProvidersFile) throws DocumentException, IOException {
		...
		if(!dataProviderElements.isEmpty()) {
        	for (Element currentDataProviderElement : (List<Element>) dataProviderElements) {
        		...
        		DataProvider provider = new DataProvider(providerId, providerName, providerCountry, providerDescription, dataSources, null);
        		for (Element currentDataSourceElement : (List<Element>) currentDataProviderElement.elements("dataSource")) {
        			...
        			if(urlString != null && !urlString.isEmpty()) {
        				...
        				dataSource = new DataSourceOai(provider, id, description, "oai_dc", "", null, new IdGenerated(), null);
        			}
	...
	private DataSourceOai getDataSourceOai(String urlString, DataProvider provider, String id, String description)
		...
		DataSourceOai dataSourceOai = new DataSourceOai(provider, id, description, metadataFormat, oaiSource, oaiSet, new IdGenerated(), null); 
...
//		}
		...
		DataProviderLoader loader = new DataProviderLoader();
		List<DataProvider> dataProviders = loader.loadDataProviders(new File("F:/dreis/Desktop/REPOX Tel/tel data - attempt2.xml"));
		RepoxContextUtil.getRepoxManager().getDataProviderManager().saveDataProviders(dataProviders);
//		List<DataSource> loadedDataSources = RepoxContextUtil.getRepoxManager().getDataProviderManager().loadDataSources();
