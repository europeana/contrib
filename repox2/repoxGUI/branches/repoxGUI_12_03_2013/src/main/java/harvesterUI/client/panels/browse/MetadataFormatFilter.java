...
public class MetadataFormatFilter extends FilterButton {
    ...
    private void reGetMetadataFormats(TreeStore<FilterAttribute> saveCheckedMetadataFormat,List<FilterAttribute> result) {
        ...
        for(FilterAttribute FilterAttribute : metadataFormatListStore.getModels()) {
            for(FilterAttribute savedAttribute : saveCheckedMetadataFormat.getModels()) {
                if(FilterAttribute.getValue().equals(savedAttribute.getValue())) {
                    metadataFormatListStore.remove(FilterAttribute);
                    checkedMetadataFormatListStore.add(FilterAttribute,false);
                    checkedMetadataFormatList.setChecked(FilterAttribute,true);
                }
            }
        }
    ...
    public void updateFilterData(){
        ...
        ((FilterServiceAsync)Registry.get(HarvesterUI.FILTER_SERVICE)).getMetadataFormats(callback);
        ...
        ((FilterServiceAsync)Registry.get(HarvesterUI.FILTER_SERVICE)).getTransformations(callback);
    }
