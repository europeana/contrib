package pt.utl.ist.repox.data.load;
 ...
 */
    ...
    // todo change it!! -> loadAggregatorsRepox and return a list of Aggregators
	public List<DataProvider> loadDataProviders(File dataProvidersFile) throws DocumentException, IOException {
		...
		if(!dataProviderElements.isEmpty()) {
        	for (Element currentDataProviderElement : (List<Element>) dataProviderElements) {
                ...
                AggregatorRepox agg = new AggregatorRepox();
                agg.setName(providerName);

        		// todo add country data provider to the constuctor
                DataProvider provider = new DataProvider(agg, providerId, providerName, providerDescription, dataSources);
        		for (Element currentDataSourceElement : (List<Element>) currentDataProviderElement.elements("dataSource")) {
                    ...
                    String language = currentDataSourceElement.attributeValue("language");
        			...
        			if(urlString != null && !urlString.isEmpty()) {
        				...
        				dataSource = new DataSourceOai(provider, id, description, "oai_dc", "", null, new IdGenerated(), null, Language.valueOf(language), null, null, -1);
        			}
        			provider.addDataSource(dataSource);
    				//dataSources.add(dataSource);
        		}
                agg.addDataProvider(provider);
	...
	private DataSourceOai getDataSourceOai(String urlString, DataProvider provider, String id, String description)
		...
		DataSourceOai dataSourceOai = new DataSourceOai(provider, id, description, metadataFormat, oaiSource, oaiSet, new IdGenerated(), null, null, null, null, -1);
...
//		}
		...
		//DataProviderLoader loader = new DataProviderLoader();
		//List<DataProvider> dataProviders = loader.loadDataProvider(new File("F:/dreis/Desktop/REPOX Tel/tel data - attempt2.xml"));
		//todo RepoxContextUtil.getRepoxManager().getDataManager().saveData(null, dataProviders);
//		List<DataSource> loadedDataSources = RepoxContextUtil.getRepoxManager().getDataManager().loadDataSources();
//		for (DataSource dataSource : loadedDataSources) {
//			dataSource.initAccessPoints();
//		}
//		RepoxContextUtil.getRepoxManager().getAccessPointsManager().initialize(loadedDataSources);
	}
}
