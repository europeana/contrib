...
public class BrowseView extends View {
    ...
    protected void handleEvent(final AppEvent event) {
        if (event.getType() == AppEvents.Init) {
            initUI();
        }else if (event.getType() == AppEvents.FilterData) {
            mainGrid.loadGridFilteredData((List<FilterQuery>)event.getData());
            ...
            AsyncCallback<Map<String,BaseModel>> callback = new AsyncCallback<Map<String,BaseModel>>() {
...
//        Registry.register("mainDataManager", mainDataManager);
    ...
    private void updateDataSet(DataSourceUI dataSourceUI, Map<String, BaseModel> result){
        // Get Status
