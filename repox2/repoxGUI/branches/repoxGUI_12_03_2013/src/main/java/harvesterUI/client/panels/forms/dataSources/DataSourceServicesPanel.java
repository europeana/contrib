...
public class DataSourceServicesPanel extends FieldSetWithClickOption {
    ...
    public void addNewRestServiceCombo(){
        ...
        Button addService = new Button(HarvesterUI.CONSTANTS.add(),new SelectionListener<ButtonEvent>() {
            @Override
            public void componentSelected(ButtonEvent ce) {
                addNewRestServiceCombo();
            }
        });
        addService.setIcon(HarvesterUI.ICONS.add());
        ...
        if(getItems().size() == 0){
            newRestServiceComboContainer.add(new LabelToolItem(HarvesterUI.CONSTANTS.executionType()), new HBoxLayoutData(new Margins(0, 5, 5, 0)));
            executionTypeCombo.setValue(executionTypeCombo.getStore().getAt(0));
            newRestServiceComboContainer.add(executionTypeCombo, new HBoxLayoutData(new Margins(0, 5, 5, 0)));
            newRestServiceComboContainer.add(addService, new HBoxLayoutData(new Margins(0, 5, 5, 0)));
        }else{
