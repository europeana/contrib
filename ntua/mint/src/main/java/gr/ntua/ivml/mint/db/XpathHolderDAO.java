...
public class XpathHolderDAO extends DAO<XpathHolder, Long> {
	...
	public List<XpathHolder> getByLikePath( XmlObject xo, String path ) {
		return (List<XpathHolder>) getSession().createQuery( " from XpathHolder where xpath like :x and xmlObject = :xo")
		.setEntity("xo", xo )
		.setString( "x", path)
		.list();
	}
