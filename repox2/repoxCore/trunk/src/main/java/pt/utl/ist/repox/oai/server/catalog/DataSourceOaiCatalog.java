    ...
    private Map getListRecords(String from, String until, String set, String metadataPrefix, int offset,
        ...
        try {
            ...
            if(set == null || set.isEmpty()){
                ...
                for(OaiSet oaiSet : dataSetIds){
                    ...
                    if(numberRecords < maxListSize && difference > 0){
                        Map dataSetMap = getRecordsFromDataSet(from, until, oaiSet.getDataSetId(), metadataPrefix,
                                ...
                                numberRecords == 0 ? -1 : Math.abs(numberRecords - maxListSize), offset, collectionSize,transformationResultLogger);
                ...
                Map dataSetMap = getRecordsFromDataSet(from,until,set,metadataPrefix,offset,fullRecord,dataSource, set,
                        -1, -1, -1, collectionSize,transformationResultLogger);
                listObjectsMap.put("resumptionMap",dataSetMap.get("resumptionMap"));
                if(fullRecord) {
                    finalItems.addAll((List<String>)dataSetMap.get("records"));
                }
    ...
    private Map getRecordsFromDataSet(String from, String until, String set, String metadataPrefix, int offset,
        ...
        for (OaiItem currentItem : oaiListResponse.getOaiItems()) { // load the headers and identifiers ArrayLists.
            try {
                ...
                StringBuffer oaiRecordHeader = new StringBuffer("<oai:header");
                if(currentItem.isDeleted()) {
                    oaiRecordHeader.append(" status=\"deleted\"");
                }
                oaiRecordHeader.append("><oai:identifier>").append(encodedIdentifier).append("</oai:identifier>").append("<oai:datestamp>")
                        .append(currentItem.getDatestamp()).append("</oai:datestamp>").append("<oai:setSpec>").append(currentItem.getSetSpec())
                        .append("</oai:setSpec></oai:header>");
                ...
                if(!fullRecord) {
                    ...
                    if(!currentItem.isDeleted()) {
                        ...
                        oaiMetadata.append("<oai:metadata>").append(xmlRecordString).append("</oai:metadata>");
                    }
                    ...
                    StringBuffer oaiRecord = new StringBuffer("<oai:record>").append(oaiRecordHeader.toString()).append(oaiMetadata.toString()).append(oaiProvenance).append("</oai:record>");
...
//	                }
        ...
        if(maxNumRecords2Return == -1){
            if(oaiListResponse.getOaiItems().size() > maxListSize) {
                ...
                dataSetMap.put("resumptionMap", getResumptionMap(resumptionToken, offsetSet.isEmpty() ? totalRecordsSize : totalSetRecords, offset));
            }
            else if((oaiListResponse.getOaiItems().size() == maxListSize && totalRecordsSize > totalSetRecords)){
...
//        }
        ...
        StringBuffer originDescription =new StringBuffer("<oai:originDescription harvestDate=\"")
                .append(oaiItem.getDatestamp()).append("\" altered=\"true\">")
                .append("<oai:baseURL>").append(dataSource.getOaiSourceURL()).append("</oai:baseURL>")
                .append("<oai:identifier>").append(oaiItem.getIdentifier()).append("</oai:identifier>")
                .append("<oai:datestamp>").append(oaiItem.getDatestamp()).append("</oai:datestamp>")
                .append("<oai:metadataNamespace>").append(metadataNamespace).append("</oai:metadataNamespace>")
                .append("</oai:originDescription>");

        oaiProvenance.append("<oai:about><oai:provenance xmlns=\"http://www.openarchives.org/OAI/2.0/provenance\"")
                ...
                .append( "</oai:provenance></oai:about>");
    ...
    public String getRecord(String identifier, String metadataPrefix) throws CannotDisseminateFormatException, IdDoesNotExistException {
        ...
        try {
            ...
            String oaiRecordHeader = "<oai:header";
            ...
            String encodedIdentifier = OAIUtil.xmlEncode(identifier);
            oaiRecordHeader = oaiRecordHeader + "><oai:identifier>" + encodedIdentifier + "</oai:identifier>"
                    + "<oai:datestamp>" + oaiItem.getDatestamp() + "</oai:datestamp>"
                    + "<oai:setSpec>" + urn.getDataSourceId() + "</oai:setSpec></oai:header>";
            ...
            String oaiMetadata = (recordRepox.isDeleted() ? "" : "<oai:metadata>" + xmlRecordString + "</oai:metadata>");
            ...
            String oaiRecord = "<oai:record xmlns:oai=\"http://www.openarchives.org/OAI/2.0/\">" + oaiRecordHeader + oaiMetadata + oaiProvenance + "</oai:record>";
    ...
    public Map listSets() {
        ...
        try {
            ...
            for (DataSourceContainer dataSourceContainer : dataSourceContainers.values()) {
                sets.add("<oai:set><oai:setSpec>" + dataSourceContainer.getDataSource().getId() + "</oai:setSpec><oai:setName>"
                        + dataSourceContainer.getDataSource().getDescription() + "</oai:setName></oai:set>");
            }
        }
        catch(Exception e) {
            log.error(e.getMessage(),e);
