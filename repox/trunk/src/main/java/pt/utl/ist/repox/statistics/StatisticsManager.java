...
import pt.utl.ist.repox.data.dataSource.IdProvided;
...
public class StatisticsManager {
    ...
    public RepoxStatistics generateStatistics() throws IOException, DocumentException, SQLException {
        ...
        List<DataProvider> allDataProviders = new ArrayList<DataProvider>();
        for (AggregatorRepox aggregatorRepox : aggregators) {
            allDataProviders.addAll(aggregatorRepox.getDataProviders());
        }
        ...
        for(DataProvider dataProvider : allDataProviders) {
            ...
            for(DataSource dataSource : dataProvider.getDataSources()) {
                ...
                if(dataSource.getDataProvider().getCountry() != null) {
                    ...
                    if(countriesRecords.get(dataSource.getDataProvider().getCountry().name()) != null) {
                        countryRecordsTotal += countriesRecords.get(dataSource.getDataProvider().getCountry().name());
                    }

                    countriesRecords.put(dataSource.getDataProvider().getCountry().name(), countryRecordsTotal);
                }
        ...
        RepoxStatistics repoxStatistics = new RepoxStatistics(dataSourcesIdExtracted, dataSourcesIdGenerated, dataSourcesIdProvided,
                aggregators.size(), dataProviders, dataSourcesOai, dataSourcesDirectoryImporter, dataSourcesMetadataFormats, recordsAvgDataSource,
    ...
    public synchronized void saveStatistics(RepoxStatistics repoxStatistics) throws IOException {
        ...
        rootNode.addElement("aggregators").setText(String.valueOf(repoxStatistics.getAggregators()));
    ...
    public synchronized RepoxStatistics loadRepoxStatistics() throws IOException, DocumentException, SQLException, ParseException {
        ...
        RepoxStatistics repoxStatistics = new RepoxStatistics(dataSourcesIdExtracted, dataSourcesIdGenerated, dataSourcesIdProvided,
                aggregators, dataProviders, dataSourcesOai, dataSourcesDirectoryImporter, dataSourcesMetadataFormats, recordAvgDataSource,
