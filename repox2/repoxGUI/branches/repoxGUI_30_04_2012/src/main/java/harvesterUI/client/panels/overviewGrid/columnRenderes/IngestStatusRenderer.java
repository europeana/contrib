...
public class IngestStatusRenderer implements GridCellRenderer<ModelData> {
    ...
    public Object render(ModelData model, String property, ColumnData config, int rowIndex, int colIndex,
        ...
        if(model instanceof DataProviderUI || model instanceof DataSourceUI) {
            final DataSourceUI dataSourceUI;
            if(model instanceof  DataProviderUI) {
                DataProviderUI dataProviderUI = (DataProviderUI) model;
                dataSourceUI = dataProviderUI.getDataSourceUIList().get(0);
            } else
            ...
            if(dataSourceUI.getStatus().equals("RUNNING") || dataSourceUI.getStatus().equals("RUNNING_SAMPLE")) {
                if(dataSourceUI.getTotalRecords() != null && dataSourceUI.getTotalRecords() != -1 && dataSourceUI.getTotalRecords() != 0){
...
//                                        Info.display("Message", "Items were loaded", "");
            ...
            }else if(dataSourceUI.getStatus() != null) {
...
//                imageButton.setStyleAttribute("float", "left");
                ...
                if(tokensType[tokensType.length-1].equals("SAMPLE")) {
                    ...
                    imageButton.setStyleAttribute("float", "left");
                    container.add(imageButton, new FlowData(0,5,0,0));
                ...
                // Special case when task failed but is retrying
                    ...
                    service.getRunningTask(dataSourceUI.getDataSourceSet(),callbackRT);
