...
//	  YYYY = four-digit year ; MM = two-digit month (01=January, etc.) ; DD = two-digit day of month (01 through 31)
	...
	public void run() {
		...
		try {
			...
			if(resumptionTokenFile.exists()) {
				StringUtil.simpleLog("Using previous resumption token", this.getClass(), logFile);
	...
	private ListRecords getListRecordsWithRetries(String resumptionToken, int retries) throws IOException, ParserConfigurationException,
		...
		try {
			ListRecords listRecords = new ListRecords(sourceUrl, resumptionToken);
			return listRecords;
		}
	...
	private void writeRequest(int currentRequest, ListRecords listRecords)
	 ...
	 * @throws Exception
	 */
	public static void runFromFile(String recordsDir, String metadataFormat, File logFile) throws IOException, ParseException, TransformerConfigurationException {
