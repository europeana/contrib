...
public class Config {
	...
	public static boolean hasCustomJsp(String originalJsp){
		File customFile=new File(Config.getRealPath("WEB-INF/custom/jsp/"+originalJsp));
	...
	public static File getProjectRoot() {
		File result = null;
		try {
		result = new File( Config.class.getResource("/mint.properties").getPath());
		result = result.getParentFile().getParentFile().getParentFile();
		} catch( Exception e) {
			log.error( "Cant find project root", e );
			return null;
		}
		if( !result.canRead()) return null;
		return result;
	}
