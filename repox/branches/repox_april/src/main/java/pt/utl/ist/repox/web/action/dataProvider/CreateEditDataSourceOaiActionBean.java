...
*/
	...
	public Resolution getAllOaiSources() throws DocumentException, IOException {
		if(dataProviderId != null) {
			dataProvider = RepoxContextUtil.getRepoxManager().getDataProviderManager().getDataProvider(dataProviderId);
		}
		else {
			...
			if(listSets == null || (listSets.getErrors() != null && listSets.getErrors().getLength() > 0)) {
                ...
                for (Node aNodeList : nodeList) {
                    ...
                    DataSourceOai dataSourceOai = new DataSourceOai(dataProvider, setId, setDescription, MetadataFormat.oai_dc.toString(),
                            dataSource.getOaiSourceURL(), setSpec, new IdGenerated(), null);
                    ...
                    if (!isDuplicate) {
                        while (RepoxContextUtil.getRepoxManager().getDataProviderManager().getDataSource(dataSourceOai.getId()) != null) {
				...
				RepoxContextUtil.getRepoxManager().getDataProviderManager().updateDataProvider(dataProvider, dataProvider.getId());
	...
	public void loadDataSource() throws DocumentException, IOException {
		dataSource = (DataSourceOai) context.getRepoxManager().getDataProviderManager().getDataSource(dataSourceId);
	}
