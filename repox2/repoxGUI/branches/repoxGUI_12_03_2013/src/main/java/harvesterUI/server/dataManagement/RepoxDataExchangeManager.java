...
import harvesterUI.server.transformations.TransformationMatcher;
...
//import pt.utl.ist.repox.dataProvider.DataProviderEuDML;
...
//            dataSourceUI.setRecords(dataSourceUI.getOldTasks().get(dataSourceUI.getOldTasks().size()-1).getRecords());
            ...
            if(task.getFirstRun().getTime().after(Calendar.getInstance().getTime()) || !task.getFrequency().name().equals("ONCE")) {
                if(task.getParameters()[1].equals(dataSourceUI.getDataSourceSet())) {
                    if(task.getTaskClass().getSimpleName().equals("IngestDataSource")) {
                        ...
                        dataSourceUI.setNextIngest(firstRunStr);
    ...
    public static void getDataSetInfo(DataSource dataSource, DataSourceUI newDataSourceUI) throws ServerSideException{
        try{
            ...
            newDataSourceUI.setRecords(dataSource.getNumberRecords());
...
//            newDataProviderUI.setEmail(((DataProviderEuDML) dataProvider).getEmail());
...
//        }
    ...
    public static List<SchemaUI> convertRepoxSchemas(TransformationMatcher transformationMatcher) throws ServerSideException{
        ...
        transformationMatcher.processSchemaStatistics();
        ...
        for(MetadataSchema metadataSchema : schemaList){
            ...
            for(MetadataSchemaVersion metadataSchemaVersion : metadataSchema.getMetadataSchemaVersions()){
                MdrDataStatistics mdrDataStatistics = transformationMatcher.getSchemaVersionsStatisticsMap().get(schemaUI.getShortDesignation() + metadataSchemaVersion.getVersion());
                ...
                if(mdrDataStatistics != null)
                    ...
                    schemaVersionUI = new SchemaVersionUI(metadataSchemaVersion.getVersion(),metadataSchemaVersion.getXsdLink(),new MdrDataStatistics(0,null));
