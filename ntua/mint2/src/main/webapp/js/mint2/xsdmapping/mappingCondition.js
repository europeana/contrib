(function($) {
	...
	function render(container) {
		...
		if(!data.settings.empty) {
			...
			var add = $("<span>")
			.addClass("mapping-condition-add")
			.attr("title", "Add condition")
			.appendTo(actions)
			.click(function () {
				self.mappingCondition("addConditionClause");
			});				

			// Add condition subclause action
			var addSub = $("<span>")
			.addClass("mapping-condition-add-subcondition")
			.attr("title", "Add condition subclause")
			.appendTo(actions)
			.click(function () {
				self.mappingCondition("addConditionClause", true);
			});

			// Remove condition action
			var remove = $("<span>")
			.addClass("mapping-condition-remove")
			.attr("title", "Remove condition")
			.appendTo(actions)
			.click(function () {
				self.mappingCondition("removeConditionClause");
			});
		...
		select.change(function () {
			self.mappingCondition("setLogicalOperator", select.val());
		});
...
//		logicalop.css("top", logicalopTop + "px");
	...
	var methods = {
		...
		setLogicalOperator: function(logicalop) {
			var data = this.data(widget);
			$.ajax({
				...
				data: {
					...
					key: "logicalop",
					value: encodeURIComponent(logicalop),
					id: data.target.id,
					path: this.mappingCondition("path"),
					index: data.settings.index
				},
				
				// response should be the complete modified condition json
				success: function(response) {
					data.element.mappingArea("setCondition", response, data.settings.index);
				}
			});			
		},
		
		addConditionClause: function(complex) {
			var data = this.data(widget);
		...
		removeConditionClause: function() {
			var data = this.data(widget);
...
(function($) {
	...
	var methods = {
		...
		setConditionClauseKey: function(key, value) {
			var data = this.data(widget);
			var path = this.mappingConditionClause("path");
			...
			$.ajax({
				...
				data: {
					...
					value: encodeURIComponent(value),
					id: data.target.id,
					path: this.mappingConditionClause("path"),
					index: data.settings.index
				},
				
				success: function(response) {
					var element = data.condition.data("mappingCondition").element;
					element.mappingArea("setCondition", response, data.settings.index);
				}
			});
		},
