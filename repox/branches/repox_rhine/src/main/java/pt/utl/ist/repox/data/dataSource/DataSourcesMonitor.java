...
public class DataSourcesMonitor {
	...
	private DataSourcesMonitor() {
		configurationFile = new File(RepoxContextUtil.getRepoxManager().getConfiguration().getXmlConfigPath(),
									RepoxContextUtil.DATA_SOURCES_STATE_FILENAME);
	}
	...
	public static DataSourcesMonitor getInstance() {
		if(monitorInstance == null) {
			...
			try {
				...
				log.error("Error loading Data Source states from file " + RepoxContextUtil.DATA_SOURCES_STATE_FILENAME, e);
			}
		}
	 ...
	 * @param dataSources
	 */
	public void monitorDataSources(List<DataSource> dataSources) {
		dataSourcesState = new TreeMap<String, Boolean>();
		...
		for (DataSource dataSource : dataSources) {
			try {
				...
				isWorking = monitorDataSource(dataSource);

				dataSourcesState.put(dataSource.getId(), isWorking);
			}
			catch(Exception e) {
