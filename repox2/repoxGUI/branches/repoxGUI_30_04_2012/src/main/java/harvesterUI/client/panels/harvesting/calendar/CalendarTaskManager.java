...
public class CalendarTaskManager extends ContentPanel {
    ...
    public void updateScheduleTasks(){
        ...
        ((TaskManagementServiceAsync) Registry.get(HarvesterUI.TASK_MANAGEMENT_SERVICE)).getCalendarTasks(callback);
    }
    ...
    private void erasePastScheduleTasks(List<ScheduledTaskUI> scheduleTaskUIs, Date today) {
        ...
        while(iter.hasNext()) {
            ...
            if(scheduledTaskUI == null || scheduledTaskUI.getDate().before(today))
                iter.remove();
        }
    }
