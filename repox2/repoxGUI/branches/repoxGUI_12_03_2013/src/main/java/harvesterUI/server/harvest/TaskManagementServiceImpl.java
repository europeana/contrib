...
//            }
//        }
        List<ScheduledTaskUI> scheduledTaskUIs = getParsedScheduledTasks(config.getOffset(),config.getLimit());
        int totalSize = RepoxServiceImpl.getRepoxManager().getTaskManager().getScheduledTasks().size();

        return new BasePagingLoadResult<ScheduledTaskUI>(scheduledTaskUIs, config.getOffset(), totalSize);
    }
