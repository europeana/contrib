...
var i = event.changedTouches[0];
			...
			$elementFromPoint1 = document.elementFromPoint(i.clientX, i.clientY);
			var pos=$elementFromPoint1.className.indexOf("schema-tree-drop");
		    if(pos>=0){
		    	$.vakata.dnd.helper.find("ins").removeClass('jstree-invalid').addClass('jstree-ok');
		    }else{$.vakata.dnd.helper.find("ins").removeClass('jstree-ok').addClass('jstree-invalid');}
		...
		case "touchend":
			...
			var pos=$elementFromPoint1.className.indexOf("schema-tree-drop");
		    if(pos>=0){
		    	$.vakata.dnd.helper.find('.jstree-icon').removeClass('jstree-er').addClass('jstree-ok');
		    }
			if (cancelMouseUp) {
				cancelMouseUp = false;
				event.preventDefault();
		...
		simulatedEvent.initMouseEvent(type, true, true, window, 1, i.screenX, i.screenY, i.clientX, i.clientY,
                false, false, false, false, button, null);

     	currentDOMElement.dispatchEvent(simulatedEvent);
    	if($elementFromPoint1 != null){
    	 var pos=$elementFromPoint1.className.indexOf("schema-tree-drop");
	    if(pos>=0){
	       // elementFromPoint1.className="jstree-hovered";
	    	simulatedEvent = document.createEvent("MouseEvent");		// This check avoids click being emulated on a double tap
			simulatedEvent.initMouseEvent("click", true, true, window, 1, i.screenX, i.screenY, i.clientX, i.clientY,false, false, false, false, button, null);
			currentDOMElement.dispatchEvent(simulatedEvent);
	        }}
		/*var touches = event.changedTouches,
			first = touches[0],
		...
		currentDOMElement.dispatchEvent(simulatedEvent);
		console.log(type+" tapValid:"+tapValid+",first.target:"+first.target+", lastTap="+lastTap);
