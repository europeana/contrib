...
public class ViewDataProviderActionBean extends SchedulingActionHelper {
	...
	private DataSource getDataSourceFromParameter() throws DocumentException, IOException {
		...
		DataSource dataSource = context.getRepoxManager().getDataProviderManager().getDataSource(dataSourceId);
		return dataSource;
	}
...
//		return view(dataSource.getDataProvider().getId());
	...
	public Resolution exportToFilesystem() throws IOException, DocumentException, NoSuchMethodException, ClassNotFoundException, ParseException {
		DataSource dataSource = getDataSourceFromParameter();
	...
	public Resolution viewLogoImage() throws DocumentException, IOException {
		String dataProviderId = context.getRequest().getParameter("dataProviderId");
		if(dataProviderId == null) {
			return returnNotFound();
		}
		DataProvider dataProvider = RepoxContextUtil.getRepoxManager().getDataProviderManager().getDataProvider(dataProviderId);
		if(dataProvider == null) {
			return returnNotFound();
		}
		final File logoFile = new File(RepoxContextUtil.getRepoxManager().getConfiguration().getLogosDir(), dataProvider.getLogo());
		if(!logoFile.exists() || logoFile.isDirectory()) {
			log.error("logoFile is not valid: " + logoFile.getAbsolutePath());
			return returnNotFound();
		}


		String mimetype = "image/" + dataProvider.getLogo().substring(dataProvider.getLogo().lastIndexOf(".") + 1);
