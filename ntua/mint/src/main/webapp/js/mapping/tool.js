...
function showButtonElements(contents, navigation) {
	hideAllPanels();
    ...
    if(item != undefined && item.children != undefined) {
		for(var i in item.children) {
			...
			if(hide != undefined && hide.indexOf(element) >= 0) {
				show = false;
			} else if(include != undefined && include.indexOf(element) < 0) {
				show = false;
			} else {
...
function showGroupElements(group, navigation) {
	showButtonElements(group.contents, navigation);
}

function showTemplateElements(navigation) {
	showButtonElements(targetDefinition.template, navigation);
}
