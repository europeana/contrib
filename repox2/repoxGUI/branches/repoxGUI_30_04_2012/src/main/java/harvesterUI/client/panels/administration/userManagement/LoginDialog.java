...
//        setIcon(HarvesterUI.ICONS.logo16());
    ...
    protected void onSubmit() {
        ...
        AsyncCallback<BaseModel> callback = new AsyncCallback<BaseModel>() {
            ...
            public void onSuccess(BaseModel loginData) {
                if(loginData.get("status").equals("succeeded")) {
                    ...
                    if(saveCookieBox.getValue()){
                        Cookies.setCookie("sid", (String)loginData.get("sessionID"), expires, null, "/", false);
                        Cookies.setCookie("userRole", (String)loginData.get("role"), expires, null, "/", false);
                        Cookies.setCookie("loggedUsrName", (String)loginData.get("userName"), expires, null, "/", false);
                    } else{
                        Cookies.setCookie("sid", (String)loginData.get("sessionID"));
                        Cookies.setCookie("userRole", (String)loginData.get("role"));
                        Cookies.setCookie("loggedUsrName", (String)loginData.get("userName"));
                    }

                    if(userName.getValue() != null){
                        Cookies.setCookie("enteredUsername" + UtilManager.getServerUrl(),userName.getValue(), expires, null, "/", false);
                        Cookies.setCookie("enteredPassword" + UtilManager.getServerUrl(),password.getValue(), expires, null, "/", false);
                    }

                    if(UtilManager.getUrlLocaleLanguage().isEmpty() || !UtilManager.getUrlLocaleLanguage().equals((String)loginData.get("language"))){
                        Cookies.setCookie("tempLang",(String)loginData.get("language"));
                        Cookies.setCookie("tempUsername",(String)loginData.get("userName"));
                        Window.Location.assign(UtilManager.getServerUrl() + "?locale=" + (String)loginData.get("language"));
                    }else{
                        // Only add the history controller for logged users
                        Dispatcher.get().addController(new HistoryController());
                        LoginDialog.this.hide();
                        HarvesterUI.UTIL_MANAGER.setLoggedUser((String)loginData.get("userName"),(String)loginData.get("role"));
    ...
    private void createRecoverPasswordForm() {
        ...
        recoverFormPanel.add(emailField, new FormData("95%"));
        ...
        Button b = new Button(HarvesterUI.CONSTANTS.send(),HarvesterUI.ICONS.send_recovery_pass_icon(),new SelectionListener<ButtonEvent>() {
            ...
            public void componentSelected(ButtonEvent be) {
                AsyncCallback<Boolean> callback = new AsyncCallback<Boolean>() {
                    ...
                    public void onSuccess(Boolean foundUserMailMatch) {
                        if(foundUserMailMatch) {
                            recoverDialog.hide();
                ...
                service.resetUserPassword(userName,email,HarvesterUI.getProjectType(), callback);
