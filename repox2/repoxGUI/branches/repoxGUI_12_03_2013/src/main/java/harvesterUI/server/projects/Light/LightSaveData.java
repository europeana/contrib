...
public class LightSaveData {
        ...
        try {
            ...
            for(TransformationUI transformationUI: dataSourceUI.getMetadataTransformations()) {
                if(transformationUI.isMdr()){
                    MetadataTransformation metadataTransformation= saveMdrTransformation(transformationUI);
    ...
    public static MetadataTransformation saveMdrTransformation(TransformationUI transformationUI) throws ServerSideException{
        ...
        String transfId = TransformationsServiceImpl.forURL(LightSaveData.decodeMDRId(transformationUI.getIdentifier()));
        String transfMime = TransformationsServiceImpl.forURL("application/xslt+xml");
        String statusUri = RepoxServiceImpl.getRepoxManager().getConfiguration().getMdrUrl()+ "/services/provider/getTranslation?uri="+transfId+"&mimetype="+transfMime;
        ...
        try {
            ...
            String newId = encodeMDRId(transformationUI.getIdentifier());
            String xslPath = newId + ".xsl";
            File file = new File(xsltDir,xslPath);
            XmlUtil.writePrettyPrint(file,document);

            MetadataTransformation metadataTransformation = new MetadataTransformation(newId,
                    transformationUI.getDescription(),transformationUI.getSrcFormat(),
                    transformationUI.getDestFormat(),xslPath,
                    false,transformationUI.getIsXslVersion2(),transformationUI.getSchema(),transformationUI.getMetadataNamespace());
            ConfigSingleton.getRepoxContextUtil().getRepoxManager().getMetadataTransformationManager().
                    saveMetadataTransformation(metadataTransformation,"");
    ...
    public static String encodeMDRId(String mdrId){
        mdrId = mdrId.replace(":","-");
        mdrId = mdrId.replace("/","$$");
        mdrId = mdrId.replace("#","$");
        return mdrId;
    ...
    public static String decodeMDRId(String mdrId){
        mdrId = mdrId.replace("-",":");
        mdrId = mdrId.replace("$$","/");
        mdrId = mdrId.replace("$","#");
        return mdrId;
    ...
    public static void main(String[] args) throws ServerSideException{
        ...
        if(transformationUI.isMdr()){
            ...
            String transfId = TransformationsServiceImpl.forURL(transformationUI.getIdentifier());
            String transfMime = TransformationsServiceImpl.forURL("application/xslt+xml");
            String statusUri = "http://digmap3.ist.utl.pt:8080/mdr/services/provider/getTranslation?uri="+transfId+"&mimetype="+transfMime;

            try {
                Document document = reader.read(new URL(statusUri));

                File file = new File("D:\\Projectos\\TESTS\\testXsd.xsl");
                XmlUtil.writePrettyPrint(file,document);
