...
public class DataSourceOaiCatalog extends AbstractCatalog {
                ...
                for (DataSourceContainer dataSourceContainer : dataSourceContainers.values()) {
                    ...
                    for (MetadataTransformation metadataTransformation : dataSourceContainer.getDataSource().getMetadataTransformations().values()) {
                        schemaLocationsMap.put(metadataTransformation.getDestinationFormat(),
                                metadataTransformation.getNamespace() + " " + metadataTransformation.getSchema());
                    }
    ...
    private Map getRecordsFromDataSet(String from, String until, String set, String metadataPrefix, int offset,
                                      ...
                                      int maxNumRecords2Return, int originalOffset) throws NoItemsMatchException{
        ...
        for (OaiItem currentItem : oaiListResponse.getOaiItems()) { // load the headers and identifiers ArrayLists.
            try {
                if(counter >= maxListSize || (maxNumRecords2Return != -1 ? counter >= maxNumRecords2Return : false)) {
                    break;
                }
                counter++;
                ...
                StringBuffer oaiRecordHeader = new StringBuffer("<header");
                if(currentItem.isDeleted()) {
                    oaiRecordHeader.append(" status=\"deleted\"");
                }
                oaiRecordHeader.append("><identifier>").append(encodedIdentifier).append("</identifier>").append("<datestamp>")
                        .append(currentItem.getDatestamp()).append("</datestamp>").append("<setSpec>").append(currentItem.getSetSpec())
                        .append("</setSpec></header>");
                ...
                if(!fullRecord) {
                    ...
                    if(!currentItem.isDeleted()) {
                        ...
                        oaiMetadata.append("<metadata>").append(xmlRecordString).append("</metadata>");
                    }
                    ...
                    StringBuffer oaiRecord = new StringBuffer("<record>").append(oaiRecordHeader.toString()).append(oaiMetadata.toString()).append(oaiProvenance).append("</record>");
...
//	                }
        ...
        if(maxNumRecords2Return == -1){
            if(oaiListResponse.getOaiItems().size() > maxListSize) {
                String resumptionToken = getResumptionToken(offsetSet, metadataPrefix, oaiListResponse.getLastRequestedIdentifier(), from, until);
                ...
                String resumptionToken = getResumptionToken(offsetSet, metadataPrefix, originalOffset + maxListSize, from, until);
...
//        }
        ...
        StringBuffer originDescription =new StringBuffer("<originDescription harvestDate=\"")
                .append(oaiItem.getDatestamp()).append("\" altered=\"true\">")
                .append("<baseURL>").append(dataSource.getOaiSourceURL()).append("</baseURL>")
                .append("<identifier>").append(oaiItem.getIdentifier()).append("</identifier>")
                .append("<datestamp>").append(oaiItem.getDatestamp()).append("</datestamp>")
                .append("<metadataNamespace>").append(metadataNamespace).append("</metadataNamespace>")
                .append("</originDescription>");

        oaiProvenance.append("<about><provenance xmlns=\"http://www.openarchives.org/OAI/2.0/provenance\"")
    ...
    public Map listSets() {
        ...
        try {
            ...
            for (DataSourceContainer dataSourceContainer : dataSourceContainers.values()) {
                sets.add("<set><setSpec>" + dataSourceContainer.getDataSource().getId() + "</setSpec><setName>"
                        + dataSourceContainer.getDataSource().getDescription() + "</setName></set>");
            }
        }
        catch(Exception e) {
            log.error(e.getMessage(),e);
