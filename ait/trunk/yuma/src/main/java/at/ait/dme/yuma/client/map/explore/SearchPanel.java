...
import at.ait.dme.yuma.client.server.FindPlaceServiceAsync;
...
public class SearchPanel extends FlowPanel {
	...
	private void doAsyncGeocoding() {
		...
		if (searchBox.getValue().trim().length() > 0) {
			service.findPlace(Application.getExternalObjectId(), 
					Application.getImageUrl(), 
					searchBox.getValue(), new AsyncCallback<FindPlaceResponse>() {
				@Override
