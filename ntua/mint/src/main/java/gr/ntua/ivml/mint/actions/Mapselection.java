...
@Results( { @Result(name = "input", location = "mapselection.jsp"),
		...
		@Result(name="download", type="stream", params={"inputName", "stream", "contentType", "application/mint", 
				  "contentDisposition", "attachment; filename=${filename}"}),
		@Result(name="download_xsl", type="stream", params={"inputName", "stream", "contentType", "application/xslt+xml", 
							  "contentDisposition", "attachment; filename=${filename}"})
...
//			setFilesize( thefilesize );
	...
	private void downloadXSL(String name, String xsl) throws Exception {
		try {
	...
	public String execute() throws Exception {
		...
		if ("createschemanew".equals(mapsel)) {
			...
			mp.setName(mapName);
			mp.setOrganization(du.getOrganization());
			...
			log.error("enableAutomaticMappings: " + Config.getBoolean("enableAutomaticMappings"));
			if(Config.getBoolean("enableAutomaticMappings")) {
				try {
					mp.applyAutomaticMappings(du);
				} catch(Exception e) {
					e.printStackTrace();
				}
			}
		...
		} else if ("downloadxsl".equals(mapsel)) {
			this.setEditMapping(0);
			if (this.getSelectedMapping() > 0) {
				boolean success = false;
				Mapping mp = DB.getMappingDAO().getById(getSelectedMapping(),
				...
				if (json != null) {
					...
					xsl = XMLFormatter.format(xslt.generateFromString(json));						
					downloadXSL(mp.getName(), xsl);
					
					return "download_xsl";
				} else {
					refreshUser();
					initLists();
					addActionError("Unable to donwload XSL from selected Mappings. Mappings are empty!");
					return ERROR;
				}
			}
