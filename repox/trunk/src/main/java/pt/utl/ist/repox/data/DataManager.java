...
public class DataManager {
    ...
    private static final int ID_MAX_SIZE = 160;
        ...
        for(DataProvider dataProvider : dataProviders) {
            ...
            if(dataProvider.getDataSources() != null && !dataProvider.getDataSources().isEmpty()) {
                for (DataSource dataSource : dataProvider.getDataSources()) {
                    ...
                    if(dataSource.getNameCode() != null) {
                        sourceNode.addElement("exportPath").setText(dataSource.getExportPath());
                    }
                    ...
                    if(dataSource instanceof DataSourceOai) {
                        ...
                        if(currentDataSource.getURLSourcesPath()!=null){
                            sourceNode.addElement("URLSourcesPath").setText(currentDataSource.getURLSourcesPath());
                        }
        ...
        for (Iterator aggIterator = root.elementIterator("aggregator"); aggIterator.hasNext();) {
            ...
            for (Iterator provIterator = currentElementAgg.elementIterator("provider"); provIterator.hasNext();) {
                ...
                for (Iterator dataSIterator = currentElementProv.elementIterator("source"); dataSIterator.hasNext();) {
                    ...
                    if(currentDataSourceElement.elementText("exportPath") != null){
                        exportPath = currentDataSourceElement.elementText("exportPath");
                    }
                    ...
                    if(dataSourceType.equals("DataSourceOai")) {
                        ...
                        DataSourceOai dataSource = new DataSourceOai(dataProvider, dataSourceId, dataSourceDescription, metadataFormat,
                                oaiSource, oaiSet, recordIdPolicy, metadataTransformations, dataSourceName, dataSourceNameCode, dataSourceIdDb, exportPath);
                        ...
                        String URLSourcesPath = currentDataSourceElement.elementText("URLSourcesPath");
                        ...
                        if(extractStrategyString.equals(Iso2709FileExtract.class.getSimpleName())) {
                            ...
                            dataSource = new DataSourceDirectoryImporter(dataProvider, dataSourceId, dataSourceDescription, metadataFormat, extractStrategy,
                                    characterEncoding, sourcesDirPath, recordIdPolicy, metadataTransformations, null, null, dataSourceName, dataSourceNameCode, dataSourceIdDb,URLSourcesPath, exportPath);
                            ...
                            dataSource = new DataSourceDirectoryImporter(dataProvider, dataSourceId, dataSourceDescription, metadataFormat, extractStrategy,
                                    sourcesDirPath, recordIdPolicy, metadataTransformations, null, null, dataSourceName, dataSourceNameCode, dataSourceIdDb,URLSourcesPath, exportPath);
                            ...
                            dataSource = new DataSourceDirectoryImporter(dataProvider, dataSourceId, dataSourceDescription, metadataFormat, extractStrategy,
                                    sourcesDirPath, recordIdPolicy, metadataTransformations, recordXPath, namespaces, dataSourceName, dataSourceNameCode, dataSourceIdDb,URLSourcesPath, exportPath);
                        }
                        dataProvider.addDataSource(dataSource);
                    }
                        ...
                        DataSourceZ3950 dataSourceZ3950 = new DataSourceZ3950(dataProvider, dataSourceId, dataSourceDescription,
                                harvestMethod, recordIdPolicy, metadataTransformations, dataSourceName, dataSourceNameCode, dataSourceIdDb, exportPath);
     ...
     * @return Status
     */
    public Status saveAggregator(AggregatorRepox newAggregator) throws IOException, DocumentException {
        List<AggregatorRepox> aggregatorsRepox = loadAggregatorsRepox();

        if(!checkIfAggregatorExists(aggregatorsRepox, newAggregator)){
            aggregatorsRepox.add(newAggregator);

            if(RepoxContextUtil.getRepoxManager().getConfiguration().getUseSipDataBase().equals("true")){
                // REPOX2SIP add aggregator
                try {
                    newAggregator.addAggregator2Database(repox2sip);
    ...
    /**
     * Check if a specific aggregator already exists in REPOX (use name and nameCode attruibutes)
     * @param aggregatorsRepox
     * @param aggregator
     * @return
     */
    private boolean checkIfAggregatorExists(List<AggregatorRepox> aggregatorsRepox, AggregatorRepox aggregator){
        for (AggregatorRepox aggregatorRepox : aggregatorsRepox) {
            if(aggregator.getName().equalsIgnoreCase(aggregatorRepox.getName()) ||
                    aggregator.getNameCode().equalsIgnoreCase(aggregatorRepox.getNameCode()))
                return true;
        }
        return false;
    }
    ...
    public void updateDataSourceId(String oldDataSourceId, String newDataSourceId)
        ...
        AccessPoint defaultTimestampAP = dataSource.getAccessPoints().get(PREFIX_INTERNAL_BD + oldDataSourceId + SUFIX_TIMESTAMP_INTERNAL_BD);
        accessPointsManager.updateDataSourceAccessPoint(dataSource, defaultTimestampAP.typeOfIndex(),
                PREFIX_INTERNAL_BD + oldDataSourceId + SUFIX_TIMESTAMP_INTERNAL_BD, PREFIX_INTERNAL_BD + newDataSourceId + SUFIX_TIMESTAMP_INTERNAL_BD);
        log.info("Updated AccessPoint with id " + PREFIX_INTERNAL_BD + oldDataSourceId + SUFIX_TIMESTAMP_INTERNAL_BD + " to " + PREFIX_INTERNAL_BD + newDataSourceId + SUFIX_TIMESTAMP_INTERNAL_BD);

        AccessPoint defaultRecordAP = dataSource.getAccessPoints().get(PREFIX_INTERNAL_BD + oldDataSourceId + SUFIX_RECORD_INTERNAL_BD);
        accessPointsManager.updateDataSourceAccessPoint(dataSource, defaultRecordAP.typeOfIndex(),
                PREFIX_INTERNAL_BD + oldDataSourceId + SUFIX_RECORD_INTERNAL_BD, PREFIX_INTERNAL_BD + newDataSourceId + SUFIX_RECORD_INTERNAL_BD);
        log.info("Updated AccessPoint with id " + PREFIX_INTERNAL_BD + oldDataSourceId + SUFIX_RECORD_INTERNAL_BD + " to " + PREFIX_INTERNAL_BD + newDataSourceId + SUFIX_RECORD_INTERNAL_BD);
