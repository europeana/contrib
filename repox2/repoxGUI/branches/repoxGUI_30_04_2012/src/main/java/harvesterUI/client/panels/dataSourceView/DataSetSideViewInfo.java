 ...
 * Time: 16:37
 */
public class DataSetSideViewInfo extends DefaultFormPanel{
    ...
    public DataSetSideViewInfo() {
        ...
        dataSetFolderViewInfo = new DataSetFolderViewInfo(formData);
        dataSetOAIViewInfo = new DataSetOAIViewInfo(formData);
        dataSetZ39ViewInfo = new DataSetZ39ViewInfo(formData);
    }
    ...
    public void createForm(final DataSourceUI dataSourceUI) {
        ...
        if(typeOfDS.equals("Folder"))
            result = dataSetFolderViewInfo.showInfo(dataSourceUI);
        if(typeOfDS.equals("OAI-PMH"))
            result = dataSetOAIViewInfo.showInfo(dataSourceUI);
        if(typeOfDS.equals("Z3950"))
            result = dataSetZ39ViewInfo.showInfo(dataSourceUI);

        add(result,formData);
        if(dataSourceUI.getRestServiceUIList().size() > 0)
            add(new ExternalServicesViewInfo(dataSourceUI));
...
//        operationsSet.setAutoHeight(true);
    ...
    private void setEditExternalServicesResults(DataSourceUI dataSourceUI){
        for(final ExternalServiceUI externalServiceUI : dataSourceUI.getRestServiceUIList()){
            final String reportUrl = externalServiceUI.getUri().substring(0, externalServiceUI.getUri().lastIndexOf("/")) + "/getReportFile?id="+dataSourceUI.getDataSourceSet();
            AsyncCallback<ExternalServiceResultUI> callback = new AsyncCallback<ExternalServiceResultUI>() {
                ...
                public void onSuccess(ExternalServiceResultUI result) {
                    ...
                    if(result.getState().equals("SUCCESS"))
                        validationContainer.add(new LabelToolItem(HarvesterUI.CONSTANTS.service()+": " + externalServiceUI.getName() + " " + HarvesterUI.CONSTANTS.wasSuccessful()),new HBoxLayoutData(new Margins(0, 5, UtilManager.SPECIAL_HBOX_BOTTOM_MARGIN, 0)));
                    else if(result.getState().equals("ERROR"))
                        validationContainer.add(new LabelToolItem(HarvesterUI.CONSTANTS.service()+": " + externalServiceUI.getName() + " " + HarvesterUI.CONSTANTS.wasNotSuccessful()),new HBoxLayoutData(new Margins(0, 5, UtilManager.SPECIAL_HBOX_BOTTOM_MARGIN, 0)));
                    else
                        // Running
                        validationContainer.add(new LabelToolItem(HarvesterUI.CONSTANTS.service()+": " + externalServiceUI.getName() + " " + HarvesterUI.CONSTANTS.running()),new HBoxLayoutData(new Margins(0, 5, UtilManager.SPECIAL_HBOX_BOTTOM_MARGIN, 0)));

                    ImageButton viewValidationResultButton = new ImageButton();
                    viewValidationResultButton.setToolTip(HarvesterUI.CONSTANTS.viewValidationReport());
