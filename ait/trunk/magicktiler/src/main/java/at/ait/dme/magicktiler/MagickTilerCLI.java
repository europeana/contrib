 ...
 * -s   tiling scheme ('tms' or 'ptif')<br>
...
public class MagickTilerCLI {
    ...
    private static String getVersion() {
        readManifest();
        return mf.getMainAttributes().getValue("App-Version");
    }

    private static String getURL() {
        readManifest();
        return mf.getMainAttributes().getValue("URL");
    }

    private static void readManifest() {
        if(mf == null) {
            Class clazz = MagickTilerCLI.class;

            String className = clazz.getSimpleName();
            String pathToThisClass = clazz.getResource(className + ".class").toString();
			pathToThisClass = pathToThisClass.replace("file:/", "file:");
            ...
            try {
	            mf = new Manifest(new URL(pathToManifest).openStream());
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
