package at.ac.arcs.dme.videoannotation.client.service
{
	...
	/** AnnoteaServiceSocket implements the AnnoteaService by using a generic socket connection
     *
     * @author Stefan Pomajbik
     * @author Christian Sadilek
     * 
     **/
	public class AnnoteaServiceSocket extends EventDispatcher implements AnnoteaService
	{
		...
		public function AnnoteaService() {
		}
    	...
    	private function sendHTTPDelete(annoteaBody:XML,servicePath:String,handler:Function,asPost:Boolean):void {
      		...
      		var uri:URI;
      		if (asPost) {
		        uri = new URI(serviceUrl+servicePath+"?_method=DELETE");
		        request = new Post();
		        request.contentType = HTTPService.CONTENT_TYPE_XML;
      		} else {
      			uri = new URI(serviceUrl+servicePath);
      			request = new Delete();
      		}

			trace("sendDelete with URI:"+uri+" asPost:"+asPost);
			client.addEventListener(HttpStatusEvent.STATUS,handler);
	  		client.addEventListener(HttpErrorEvent.ERROR,handler);
    	...
    	private function sendHTTPPut(annoteaBody:XML,servicePath:String,handler:Function,asPost:Boolean):void {
      		var client:HttpClient = new HttpClient();
	        var uri:URI = new URI(serviceUrl+servicePath);
	  		var data:ByteArray = new ByteArray();
			trace("sendPut with URI:"+uri+" asPost:"+asPost+", BODY["+annoteaBody+"]");
			client.addEventListener(HttpStatusEvent.STATUS,handler);
	  		client.addEventListener(ErrorEvent.ERROR,handler);
      		data.writeUTFBytes(annoteaBody);
      		data.position = 0;
			if (asPost) {
		        uri = new URI(serviceUrl+servicePath+"?_method=PUT");
		        client.post(uri, data, HTTPService.CONTENT_TYPE_XML);
      		} else {
      			uri = new URI(serviceUrl+servicePath);
	      		client.put(uri, data, HTTPService.CONTENT_TYPE_XML);
      		}
	    }
        ...
        public function updateAnnotation(_videoAnnotation:VideoAnnotation,handler:Function):void {
        	trace("updateAnnotation called with: "+_videoAnnotation);
         	sendHTTPPut(RdfXmlAnnotationBuilder.annoToRdfXml(_videoAnnotation),ANNOTEA_PATH+encodeURI(encodeURIComponent(_videoAnnotation.id)),handler,true);
        }
        ...
        public function deleteAnnotation(_videoAnnotationId:String,handler:Function):void {
        	trace("deleteAnnotation called with: "+_videoAnnotationId);
         	sendHTTPDelete(null,ANNOTEA_PATH+encodeURI(encodeURIComponent(_videoAnnotationId)),handler,true);
        }
        ...
        public function findAnnotationById(_videoAnnotationId:String,handler:Function):void {
        	trace("findAnnotationById called with: "+_videoAnnotationId);
         	sendHTTPGet(null,ANNOTEA_PATH+encodeURI(encodeURIComponent(_videoAnnotationId)),handler);
        }
		...
		public function findAnnotationBodyById(_videoAnnotationId:String,handler:Function):void {
        	trace("findAnnotationBodyById called with: "+_videoAnnotationId);
         	sendHTTPGet(null,ANNOTEA_PATH+ANNOTEA_BODY_PATH+encodeURI(encodeURIComponent(_videoAnnotationId)),handler);
        }
