...
public class XMLNodeDAO extends DAO<XMLNode, Long> {
	...
	public List<XMLNode> getStatelessByXpathHolder( XpathHolder xp, long from, long count ) {
		List<XMLNode> l =  DB.getStatelessSession().createSQLQuery( "select * from xml_node_" +
				xp.getXmlObject().getDbID() +
				" where xpath_summary_id = :xpath and xml_object_id = :xo " +
				"order by xml_node_id asc")
			.addEntity( XMLNode.class )
	...
	public XMLNode getStatelessByIdObject( XmlObject obj, Long id ) {
		XMLNode x = (XMLNode) DB.getStatelessSession().createSQLQuery( "select * from xml_node_" +
				obj.getDbID() +
				" where xml_node_id = :nodeId" )
			.addEntity( XMLNode.class )
			.setLong("nodeId", id )
	...
	public List<XMLNode> quickOtherSiblings( XMLNode tree ) {
		...
		try {
			l = DB.getStatelessSession().createSQLQuery("select * from xml_node_" +
		...
		for( XMLNode node: l ) {
			node.setXpathHolder(
					DB.getXpathHolderDAO().findById(
							node.getXpathHolder().getDbID(), false));
		}
