...
import at.ait.dme.yuma.client.server.exception.GeocoderException;
...
public class GeocoderServiceImpl extends RemoteServiceServlet  implements GeocoderService {	
	...
	public String getPlacename(String europeanaUri, String mapUrl, XYCoordinate c) 
	...
	public SemanticTag[] getTags(String europeanaUri, String mapUrl, XYCoordinate lowerLeft, 
			XYCoordinate upperRight) throws GeocoderException {
		try {
	...
	private SemanticTag[] parseGeonamesResponse(CharSequence xml) throws Exception {
        ...
        for (int i=0; i<nodes.getLength(); i++) {
        	...
        	if (toponymName != null && country != null && geonameId != null && lat != null && lon != null)
        		tags.add(new SemanticTag(toponymName, "Place", "en", toponymName + ", " + country + " (city) \nLatitude: " + lat + " \nLongitude: " + lon, GEONAMES_URI_TEMPLATE.replace("@id@", geonameId)));
        }
			...
			for (String country : countries) {
		        ...
		        if (nodes.getLength() > 0) {
		        	...
		        	if (toponymName != null && geonameId != null)
		        		tags.add(new SemanticTag(country, "Place", "en", toponymName + " (country)", GEONAMES_URI_TEMPLATE.replace("@id@", geonameId)));
		        }
			}
			return tags;
		} catch (Exception e) {
