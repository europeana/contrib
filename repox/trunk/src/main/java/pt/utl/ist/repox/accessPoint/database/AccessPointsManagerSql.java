...
public class AccessPointsManagerSql extends AccessPointsManager {
    ...
    private void initDataSource(Connection con, DataSource dataSource, boolean isUpdate) {
        ...
        for (AccessPoint ap : dataSource.getAccessPoints().values()) {
            ...
            if(!tableExists(table, con)) {
                ...
                if(tableExists(table.substring(DataManager.PREFIX_INTERNAL_BD.length()), con)) {
                    // rename the access point
                    AccessPoint defaultTimestampAP = dataSource.getAccessPoints().get(DataManager.PREFIX_INTERNAL_BD + dataSource.getId() + DataManager.SUFIX_TIMESTAMP_INTERNAL_BD);
                    AccessPoint defaultRecordAP = dataSource.getAccessPoints().get(DataManager.PREFIX_INTERNAL_BD + dataSource.getId() + DataManager.SUFIX_RECORD_INTERNAL_BD);

                    try {
                        updateDataSourceAccessPoint(dataSource, defaultTimestampAP.typeOfIndex(), dataSource.getId() + DataManager.SUFIX_TIMESTAMP_INTERNAL_BD, DataManager.PREFIX_INTERNAL_BD + dataSource.getId() + DataManager.SUFIX_TIMESTAMP_INTERNAL_BD);
                        updateDataSourceAccessPoint(dataSource, defaultRecordAP.typeOfIndex(), dataSource.getId() + DataManager.SUFIX_RECORD_INTERNAL_BD, DataManager.PREFIX_INTERNAL_BD + dataSource.getId() + DataManager.SUFIX_RECORD_INTERNAL_BD);
    ...
    private Collection getFieldFromDataSource(DataSource dataSource, String fromDateString, String toDateString,
        ...
        String recordTable = DataManager.PREFIX_INTERNAL_BD + dataSource.getId() + DataManager.SUFIX_RECORD_INTERNAL_BD;
        String timestampTable = DataManager.PREFIX_INTERNAL_BD + dataSource.getId() + DataManager.SUFIX_TIMESTAMP_INTERNAL_BD;
