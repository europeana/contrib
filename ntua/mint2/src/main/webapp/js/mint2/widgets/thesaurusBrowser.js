...
ThesaurusBrowser.prototype.render = function(containerId) {
	...
	this.languages = Mint2.languageSelection({
		onChange: function() {
		...
		success : function(response) {
			if (response != undefined) {
				...
				for ( var i in response.languages) {
					...
					var label = lang;
					if (Mint2.languages.list[lang] != undefined)
						label = Mint2.languages.labelForCode(lang, true);
...
ThesaurusBrowser.prototype.loadConceptsWithQuery = function(originalQuery, addToHistory) {
	...
	var query = $.extend({}, {
		...
		property: this.options.thesaurus.property,
