 ...
 *
 */
public abstract class AccessPointsManager {
	private static final Logger log = Logger.getLogger(AccessPointsManager.class);
	 ...
	 */
	public void processRecord(DataSource dataSource, RecordRepox record) throws IOException, SQLException {
		...
		for (AccessPoint accessPoint : dataSource.getAccessPoints().values()) {
			...
			updateIndex(record, vals, accessPoint);
     ...
     * @throws java.sql.SQLException
	 */
	public void processRecords(DataSource dataSource, List<RecordRepox> records) throws IOException, SQLException {
		...
		for (AccessPoint accessPoint : dataSource.getAccessPoints().values()) {
			...
			updateIndex(dataSource, records, vals, accessPoint);
	 ...
	 */
	public void deleteRecord(Urn recordUrn) throws IOException, DocumentException, SQLException {
		DataSource dataSource = RepoxContextUtil.getRepoxManager().getDataProviderManager().getDataSource(recordUrn.getDataSourceId());
		for (AccessPoint accessPoint : dataSource.getAccessPoints().values()) {
			deleteFromIndex(recordUrn, accessPoint);
		}
	}
     ...
     * @throws org.dom4j.DocumentException
	 */
	public void removeRecord(Urn recordUrn) throws IOException, DocumentException, SQLException {
		DataSource dataSource = RepoxContextUtil.getRepoxManager().getDataProviderManager().getDataSource(recordUrn.getDataSourceId());
		for (AccessPoint accessPoint : dataSource.getAccessPoints().values()) {
			removeFromIndex(recordUrn, accessPoint);
		}
	}
	...
	/**do some initialization tasks if necessary. This is called from the RepoxManager when initializing
     * @param dataSources
     * @throws java.sql.SQLException
	 */
	public abstract void initialize(Collection<DataSource> dataSources) throws SQLException;
     ...
     * @throws java.sql.SQLException
	 */
	protected abstract void updateIndex(RecordRepox record, Collection values, AccessPoint accessPoint) throws SQLException, IOException;
	...
	protected abstract void updateIndex(DataSource dataSource, List<RecordRepox> records, List values,
			AccessPoint accessPoint) throws SQLException, IOException;
     ...
     * @throws org.dom4j.DocumentException
	 */
	protected abstract void deleteFromIndex(Urn recordUrn, AccessPoint accessPoint) throws DocumentException, SQLException, IOException;
     ...
     * @throws java.sql.SQLException
     * @throws org.dom4j.DocumentException*/
	protected abstract void removeFromIndex(Urn recordUrn, AccessPoint accessPoint) throws SQLException, IOException, DocumentException;
