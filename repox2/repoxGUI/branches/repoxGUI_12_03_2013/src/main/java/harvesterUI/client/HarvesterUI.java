    ...
    private void checkWithServerIfSessionIdIsStillLegal(final String sessionId) {
        AsyncCallback<String> acSessionValid = new AsyncCallback<String>(){
            ...
            public void onSuccess(String userRole) {
                ...
                if(UtilManager.getUrlLocaleLanguage().isEmpty() || !UtilManager.getUrlLocaleLanguage().equals(language))
...
//            }
            ...
            }else if(historyToken.equals("MDR"))
    ...
    public static boolean useMdr(){
        String mdrUrl = (String)Registry.get("MDR_URL");
        return mdrUrl != null && !mdrUrl.isEmpty();
    }
    ...
    private void startRepoxData(){
        // Get Project Version
        AsyncCallback<BaseModel> projectTypeCall = new AsyncCallback<BaseModel>(){
            public void onFailure(Throwable caught) {
                Window.alert("Error: session could not be loaded.");
            }
            public void onSuccess(BaseModel initialData) {
                Registry.register("PROJECT_TYPE", initialData.get("PROJECT_TYPE"));
                Registry.register("MDR_URL", initialData.get("MDR_URL"));
